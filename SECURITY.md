# Security Policy

The security of KeyClickOverlay and its users is taken seriously.

Responsible disclosure helps protect users while allowing sufficient time to investigate and resolve confirmed vulnerabilities before they are made public.

<br>

## Supported Versions

Security updates are provided for the latest released version of KeyClickOverlay.

If you discover a security issue in an older version, please verify whether the issue is still present in the latest release before reporting it.

## Reporting a Vulnerability

If you believe you have discovered a security vulnerability in KeyClickOverlay, please do **not** open a public GitHub issue.

Instead, please report it privately by contacting:

**<suus@vertexcuriosity.com>**

Please include:

- A description of the vulnerability.
- Steps to reproduce the issue.
- The affected version of KeyClickOverlay.
- Any proof-of-concept or supporting information, if available.

I will acknowledge receipt of your report as soon as reasonably possible and investigate the issue.

If the vulnerability is confirmed, I will work to resolve it as quickly as reasonably possible. If the issue originates from a third-party dependency, I will coordinate with the maintainers of the affected project where appropriate.

## Scope

This policy applies only to the official KeyClickOverlay project and its published releases.
